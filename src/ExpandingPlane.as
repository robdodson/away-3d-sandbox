package 
{
	...
	public class ExpandingPlane extends Sprite
	{
		...
		private function setupMaterials():void
		{
			...
			sourceClip.graphics.beginFill(0x000000, 0);
			sourceClip.graphics.drawRect(0, 0, 1024, 1024);
			sourceClip.addChild(new Snippet());
		...
		private function setupEventListeners():void
		{
			...
			// Listen for events on the plane
		...
		private function onClick(e:MouseEvent3D):void
		{
			if (plane.scaleX <= 1)
			{
				TweenLite.to(plane, 1, { scaleX: 2, scaleZ: 2, ease: Back.easeInOut });
			}
			else
			{
				TweenLite.to(plane, 1, { scaleX: 1, scaleZ: 1, ease: Back.easeInOut });
			}
		}
