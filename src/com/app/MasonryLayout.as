package com.app
{
	...
	public class MasonryLayout
	{
		...
		private var possibleBiasArrays:Array;
		...
		public function MasonryLayout(_colCount:uint=12, _rowCount:uint=8, _cellWidth:uint=128, _cellHeight:uint=128, _cellPaddingX:uint=0, _cellPaddingY:uint=0)
		{
			...
			*/
			...
			if (whichApproach == "BIAS"){
				possibleBiasArrays = new Array();
				possibleBiasArrays[0] = new Array(RIGHT,LEFT,DOWN,UP);
				possibleBiasArrays[1] = new Array(LEFT,RIGHT,UP,DOWN);
						...
						if (i == 0){
							...
							if (!bias_findItemAPlace(items[i])){
								...
								if (positioning=="centeredsnap"){
									...
									//trace("cY: "+cY+"  maxY: "+mY+"  Math.abs(-cY + mY): "+ Math.abs(-cY + mY) +"  (cY +  Math.abs(-cY + mY)): "+(cY +  Math.abs(-cY + mY)));
									for (j=0; j < i; j++){
										items[j].x -= cX;
										items[j].y -= (cY + Math.abs(-cY + mY));
									}
